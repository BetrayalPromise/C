...
int main(int argc, char ** argv) {
    void * _defer_stack[10]; void * _ddone; int _defer_stack_index = 0;;
    int * p = malloc(sizeof(struct A));
    if (!p)
        return 1;

    if(_defer_stack_index - 1 > 10) { printf("DEFER_MAX too smaller"); }; _defer_stack[_defer_stack_index ++] = && _defer_label_52; if (0) { _defer_label_52:{ printf("defer \n"); free(p); }; if (_defer_stack_index --) goto *_defer_stack[_defer_stack_index]; else goto *_ddone; };



    printf("malloc memory\n");
    _ddone = && _defer_label_54; if (_defer_stack_index--) goto *_defer_stack[_defer_stack_index]; _defer_label_54:;
    return 0;
}
